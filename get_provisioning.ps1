...
try {
  ...
  $LogicalProcessors             = Get-WmiObject Win32_processor | Select-Object -ExpandProperty NumberOfLogicalProcessors
  $CPU_Cores                     = 0
  foreach ($p in $LogicalProcessors){
    $CPU_Cores                   = $p + $CPU_Cores
  }
  $Memory_MB                     = (Get-CimInstance Win32_PhysicalMemory | Measure-Object -Property capacity -Sum).sum / 1GB * 1024
  $Storage                       = Get-WmiObject Win32_LogicalDisk | ? {$_. DriveType -eq 3}
  $Provisioned_Storage_GB        = 0
  $Used_Storage_GB               = 0
  foreach ($s in $Storage){
    $Provisioned_Storage_GB      = $s.Size / 1GB + $Provisioned_Storage_GB
    $Used_Storage_GB             = $s.FreeSpace / 1GB + $Used_Storage_GB
  }
  ...
  $Cpu_String                    = (Get-WmiObject Win32_processor).Name
}
catch {
...
[string]::Concat("Used Storage (GB)       :" , $Used_Storage_GB)
